...
export type ServiceStatus = "upcoming" | "completed" | "overdue" | "scheduled";
...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  const statusConfig: Record<
    ServiceStatus,
    { color: string; text: string; icon: React.FC<any> }
  > = {
  ...
  return (
    <div className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`${config.color} px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1`}
        >
        ...
        <div className="mt-3 pt-3 border-t border-gray-100">
          ...
          <div className="flex gap-2 flex-wrap">
            {service.documents.map((doc, i) => (
              <span
                key={i}
                className="bg-gray-100 px-2 py-1 rounded text-gray-700 text-xs border border-gray-200"
              >
                {doc}
              </span>
            ))}
